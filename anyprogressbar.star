...
def to_rgb(color, combine=None, combine_level=0.5):
    ...
    if combine != None:
        combine_color = lambda v0, v1, level: min(
            max(int(math.round(v0 + float(v1 - v0) * float(level))), 0), 255
        )
...
def render_progress(item, padding, frame_info):
    stack_children = [
        render.Box(
            width=C_DISPLAY_WIDTH, height=C_HEIGHT + padding, color=to_rgb(C_BACKGROUND)
        ),
    ]
    ...
    if progress_value != None:
        ...
        progress_width = C_MIN_WIDTH + int(
            math.round(
                float(C_DISPLAY_WIDTH - C_MIN_WIDTH) * progress * frame_info["progress"]
            )
        )
...
def render_frame_item(items, i, fr):
    ...
    progress = math.pow(
        math.sin(0.5 * math.pi * relative_frame / C_ANIMATION_FRAMES), 2
    )
...
def render_frame(items, fr):
    children = [render_frame_item(items, i, fr) for i in range(len(items))]
...
def main(config):
    ...
    frames += (
        C_INFINITE_PROGRESS_FRAMES - (frames % C_INFINITE_PROGRESS_FRAMES)
    ) % C_INFINITE_PROGRESS_FRAMES
    return render.Root(
        delay=C_ANIMATION_DELAY,
        child=render.Box(
            child=render.Animation(
                children=[render_frame(items, fr) for fr in range(frames)],
            ),
        ),
    )
